...
export class FilmsService {
  ...
  addFilmToDB(formValue: { title: string, real: string, description: string, imageUrl: string, date: string, location?: string }): Observable<Film> {
  ...
  getAllFilmsByNewest(): Observable<Film[]> {
    //Trier par date du plus recent au plus vieux
    return this.getAllFilmsFromDB()
      .pipe(
        map(films => [...films].sort((a, b) => new Date(b.createdDate).getTime() - new Date(a.createdDate).getTime())));
  }

  getAllFilmsByOldest(): Observable<Film[]> {
    //Trier par date du plus vieux au plus récent
    return this.getAllFilmsFromDB()
      .pipe(
        map(films => [...films].sort((a, b) => new Date(a.createdDate).getTime() - new Date(b.createdDate).getTime())));
  }

  getAllFilmsByNoteToLower(): Observable<Film[]> {
    //Trier par note meilleure à pire
    return this.getAllFilmsFromDB()
      .pipe(
        map(films => [...films].sort((a, b) => new Number(a.stars).valueOf() - new Number(b.stars).valueOf())));
  }

  getAllFilmsByNoteToUpper(): Observable<Film[]> {
    //Trier par note pire à meilleure
    return this.getAllFilmsFromDB()
      .pipe(
        map(films => [...films].sort((a, b) => new Number(b.stars).valueOf() - new Number(a.stars).valueOf())));
  }
